...
const Pokedex = ({ open, onClose, pokemonData }: { open: boolean, onClose: any, pokemonData: IPokemon[] | undefined }) => {
    ...
    return (
        <div onClick={onClose} className={`z-50 fixed inset-0 flex justify-center transition-colors ${open ? 'visible bg-gradient-to-br from-indigo-950 to-slate-950' : 'invisible'}`}>
            <div onClick={(e) => e.stopPropagation()} className={`overflow-auto bg-[conic-gradient(at_top_left,_var(--tw-gradient-stops))] from-violet-500 via-indigo-200 to-purple-100 h-96 rounded-xl shadow my-auto p-8 transition-all ${open ? 'scale-100 opacity-100' : 'scale-125 opacity-0'}`}>
